...
func (r *expenseReportRepository) GetByID(id uint) (*models.ExpenseReport, error) {	var report models.ExpenseReport
...
func (r *expenseReportRepository) RemoveExpenses(reportID uint, expenseIDs []uint) error {
	var expenses []models.Expense
	if err := r.db.Where("id IN ?", expenseIDs).Find(&expenses).Error; err != nil {		return err
	}
...
func (r *expenseReportRepository) UpdateTotal(reportID uint, total float64) error {
	return r.db.Model(&models.ExpenseReport{}).		Where("id = ?", reportID).
